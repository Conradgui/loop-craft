...
- `mainline`：直接增加用户可用能力，优先获得开发时间；当前主线是 From-scratch Demo → Skill Upgrade → Conversation Distiller。
- `validation`：只验证主线刚完成的具体风险；没有对应主线交付时，不单独启动。
- `support`：看板、来源、治理和记录；不能阻塞主线，也不能因为支持工作完成而提高产品完成度。
- `not_now`：Runtime、Override、Subloop、Library Edition、发布、多平台和非阻塞 hardening；不进入当前执行队列。

一次只允许一个 `mainline` 模块处于 active。分支验证必须依附于该模块，支持任务必须让位于该模块。看板的 `work_lanes` 必须反映这四层，不能把所有任务放进同一个待办列表。
