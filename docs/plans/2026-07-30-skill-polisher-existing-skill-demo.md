...
- [ ] 获取 `Conradgui/skill-polisher` 到忽略目录，checkout detached 到完整 SHA；记录 `git rev-parse HEAD`。
- [ ] 确认 canonical root 恰为 `skills/skill-polisher/`，列出根条目、链接/ junction 和文件摘要，不执行目标脚本。
- [ ] 渐进读取 `SKILL.md`、`agents/openai.yaml`、两个直接链接的 references 和许可证；只有会改变 Finding/Preserve/边界时才读取更多。
- [ ] 形成 Skill-to-Loop Decision Record：唯一候选 `STL-001`、预期 verdict `embedded_loop`、证据状态 `Inferred`；逐项应用七项 Loopability Gate。
- [ ] 若出现多 Loop、语义损失、身份不一致或 unsupported root，诚实停在 Assessment；不得为了产物放宽安全规则。

Expected: revision、canonical root、合同和 Decision Record 都能从目标字节追溯；目标目录未被修改。

### Task 2: 锁定已批准 Candidate 并生成三个受审输入

- [ ] 将已批准设计映射为 1-loop Candidate Review：Review 只读；用户选择 `STL-001` 后允许最小本地 Polish；Recheck 使用同一 Finding 与证据；最多两次 Polish/两次 Recheck；新 Finding、扩大权限或第三轮必须停机。
- [ ] 在 `inputs/accepted-definition.json` 写 `skill-package-v0.1` 定义，identity id 必须为 `skill-polisher`。
- [ ] 用 Core canonical digest 生成 `inputs/entry-evidence.json`：`entry_type: existing_skill`、`kind: skill_assessment`、安全 source IDs、无绝对路径或原始 Skill payload。
- [ ] 从 `loop-craft/` 运行正式 inventory：
...
- [ ] 审阅 manifest 的 sorted paths、actions、file digests 与 source digest，并重新核对 pinned revision。

Expected: inventory 退出 `0`。若 canonical root 真实因允许范围内的常见条目失败，才进入 Task 3；若因选错根目录失败，只修正调用。

### Task 3（条件式）: 仅在真实失败时最小修复 LC-009

- [ ] 写一个只复现该真实 root 条目的失败测试，并绑定现有链接越界负例。
- [ ] 运行定向 RED：
...
- [ ] 只扩展被真实证明必要的标准 root；链接/junction 仍无条件拒绝，未知 root 仍 fail closed。
- [ ] 运行同一文件定向 GREEN；不运行全量套件。
- [ ] 回到 Task 2，重新创建新 manifest 路径。

Expected: 若 inventory 本来通过，本 Task 标记“不触发”，不修改 Core 或测试。

### Task 4: 真实 source-preserving build 与确定性验证

- [ ] 从 `loop-craft/` 构建到全新输出目录：
...
- [ ] 检查 Artifact 含完整原 Skill、追加 `## Feedback Loop`，Evidence 独立且包含 definition、source manifest、Entry Evidence 和 bindings。
- [ ] 运行 clean verify，预期退出 `0` 且 `status: clean`。
- [ ] 查找现有官方 Skill validator；仅在缺失时把 Skill Creator Pro 工具依赖获取到 ignored build 目录。以 `PYTHONUTF8=1` 对 Artifact 运行 validator。
- [ ] 复制 build 到独立 tampered 目录，只修改 tampered Artifact 一字节，运行 drift verify，预期退出 `1` 且不写回。
- [ ] 比较 source revision/digest，证明目标源未变。

### Task 5: 一个隔离行为 smoke case

- [ ] 在 ignored `smoke/fixture/` 创建一个极小本地 Skill，包含一个已知但不向被测模型泄漏的 Review/修改权限矛盾。
- [ ] 确认 `codex exec` 可在新上下文运行；只把生成 Artifact、fixture 路径和 Review-only 请求交给它，禁止编辑，保存 Decision Brief。
- [ ] 从输出选择一个真实 Finding ID，再启动第二个干净上下文，明确授权仅该 Finding 的本地修改，并要求 Polish → Recheck；不提供预期缺陷或 intended fix。
- [ ] 检查：Review 无写入；修改仅在授权 fixture；Recheck 复用 Finding 和证据；最多两轮；无远端、安装、发布或其它副作用。
- [ ] 若 Codex CLI 不可用或认证失败，记录为明确的行为验证 blocker；确定性 build 不能冒充 smoke 通过。

Expected: 同一个 smoke case 完成 Review 与一次授权后的 Polish/Recheck，或诚实记录外部执行器 blocker。

### Task 6: 证据记录、看板与本地提交

- [ ] 创建执行记录，包含 revision、source digest、Decision Record、Candidate Review 摘要、definition digest、manifest 摘要、build/verify/validator/drift/smoke 结果、目标源无修改证明和未解决风险。
- [ ] 更新 `dashboard/status.json`：
...
- [ ] 按 `superpowers:verification-before-completion` 复核所有成功声明。
