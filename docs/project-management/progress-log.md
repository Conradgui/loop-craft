...
## 2026-07-22：三入口共享 Loopability Gate 对齐

- 新增 `references/loopability-gate.md` 作为七项 Gate 与 0 / 1 / unsupported 分类的唯一 owner；三个入口改为直接链接，不再复制或间接借用另一入口的 Gate 正文。
- From-scratch 移除产品路由中的 `core-slice-v0.1`，获批的 0-loop Workflow 和兼容 1-loop bounded Loop 均映射到 `skill-package-v0.1`；多 Loop 或语义损失仍停在 Assessment only。
- Candidate Review 已区分 0-loop Workflow packet 与 1-loop bounded Loop packet。0-loop 的 must-preserve constraints 必须落入现有 `authority`、`workflow.steps` 或 `workflow.failure_or_stop`；只有 1-loop 审阅并写出 `loops[0].invariants`。两类共同保留 authority、boundary 和 approval scope。
- Existing Skill 的入口特定 verdict 保持不变：`keep_as_skill` 不构建无意义替代包；获批的单 Loop upgrade 继续使用 source-preserving overlay。共享的是 Gate、Review、Compiler / Adapter 合同，不是三入口的产物形态。
- TDD RED：`tests/integration/test_loop_craft_skill.py` 为 `3 failed, 6 passed`，准确命中共享 Gate 缺失、From-scratch 旧 profile、Review packet 未分型；旧 `accepted Behavior Contract` 固定短语断言已改为入口链接、明确批准与 Candidate Behavior Contract 的行为检查。
- GREEN：同一集成文件为 `9 passed`；现有真实 0-loop Packaging build + clean verify 用例为 `1 passed`。Packaging Python 未改，因此没有运行整份 9 项 Adapter 测试，也没有运行全量测试或修复无关旧文案断言。
- 本模块没有修改 Core、Schema、Adapter 或 `dashboard/status.json`。dashboard 等待主控独立复核后更新，不能由实现 Agent 的定向测试外推为阶段完成。
- 独立质量复核提出 2 个 Important：Upgrade 未实际调用 shared Candidate Review，以及 0-loop 被错误要求独立 invariants。修复后 Upgrade 以 Decision Record 进入 Review、复用已有答案并在批准后才 definition/inventory；0-loop 约束改为映射现有字段，不扩 Schema/Core。本轮合同测试 RED 为 `3 failed, 6 passed`，修复后 GREEN 为 `9 passed`；按范围没有重跑 Packaging。

## 2026-07-22：P0-02 Entry Evidence Manifest Binding

- 新增 `entry-evidence-v0.1` exact-shape 合同和 loader/validator：支持 `from_scratch / existing_skill / conversation`，固定来源摘要类型、0/1 Loop Candidate Review 分类、local-only approval scope 和 canonical definition digest binding。
- `build --entry-evidence` 在创建 output 前加载并验证输入；Evidence 写入 canonical `entry-evidence.json`，Build Manifest 绑定 `entry_evidence_digest` 和 `entry_type`，Artifact 不含该记录。
- `verify` 将 Source Package 与 Entry Evidence 作为两组独立可选 binding，从 Manifest 动态推导 Evidence 文件集合；覆盖 clean、tamper、missing、不完整字段组和 source+entry 组合。无参数历史 build 继续 clean verify。
- 三个获批入口均改用 `--entry-evidence`；Conversation 删除根目录补充 Markdown 路线。文档明确只提供结构化摘要和安全 source IDs，禁止 raw conversation、raw payload、绝对路径、私有源材料及开发记录。
- 隐私边界：validator 不做 PII 扫描、事实真实性判断、完整去敏证明或身份认证；Entry Evidence 不是第二套 IR，也不自动抽取来源材料。
- 独立质量复核发现 Candidate Review 仅有 classification、以及 `=` / `[` / `:` 后绝对路径可漏检。修复后 `candidate_review` exact shape 同时要求 bounded `summary`，但不复制 accepted definition；路径检查改用不消耗前置字符的负向边界，并保留普通 HTTPS URL。
- TDD RED：Entry 单元测试首先因新模块缺失而 collection error；pipeline 为 `6 failed, 1 passed`，准确命中 CLI 参数缺失；Skill 合同为 `1 failed`，准确命中文档未绑定。新增多 Loop 回归先得到 `DID NOT RAISE`，随后改为显式 0/1 分类映射。
- GREEN 中间证据：Entry 单元测试在最终路径边界补充前为 `25 passed`；Entry pipeline `7 passed`；Entry + Evidence package 单元测试在多 Loop 回归前为 `30 passed`；Skill 新合同测试 `1 passed`。独立复核修复后的 fresh 定向集合（Entry unit/package/integration + 五个入口 Skill 合同，包含一次 source+entry 组合 build）为 `44 passed in 4.17s`；官方 validator 输出 `Skill is valid!`；`git diff --check` 退出 `0`。
- 本模块没有修改 `dashboard/status.json`；看板仅由主控在独立复核批准后更新。

## 2026-07-23：Drift link/junction 断言同步

- 仅修改 `tests/unit/test_drift.py` 中 5 个过时的 `match="symlink"`，改为验证稳定的 `link` 拒绝行为；生产 Pipeline 未改。
- RED：5 个精确测试节点全部因异常文案为 `link or junction` / `links or junctions` 而失败。
- GREEN：完整 drift 定向文件 `38 passed`；`git diff --check` 通过。未运行与该测试合同无关的全量套件。

## 2026-07-23：Repository Delivery Documentation
