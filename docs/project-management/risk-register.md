...
| R-003 | P1 | Task 6 已验证 Manifest 关键绑定；Task 7 两次 Pipeline 构建的完整文件树和 Manifest 相同。 | 单次绑定与 deterministic dual-build 已有执行证据；drift 与最终阶段出口仍未验证。 | Task 6-7 子范围已验证；待完成 drift 和阶段出口核对。当前：整体仍 OPEN。 |
| R-004 | P1 | Task 6 已验证写入前隔离/摘要拒绝；Task 7 在 output parent 内 staging，非法定义和 Adapter 失败不留下正式 output，dangling symlink 占用路径不被覆盖。 | 正常提交和已测试失败路径已有证据；普通已有输出、Evidence 写入中途失败、强杀和非本地 FS 原子性仍未验证。 | Task 6-7 子范围已验证；待补剩余失败路径、drift 与阶段出口。当前：部分验证，整体仍 OPEN。 |
...
| R-012 | P2 | Task 7 的 symlink 回归已关闭，但受限 Windows 可能无法创建 symlink；真实 CLI smoke 尚未自动化，普通已有输出与 Evidence 部分写失败没有回归；强杀及非本地文件系统的 replace 原子性未验证。 | 当前本地文件系统的正常路径、非法定义、Adapter 失败和 dangling symlink 已验证；剩余环境/故障模式可能降低回归可移植性或留下部分 staging，但没有证据支持更强原子性声明。 | 后续把 CLI smoke、普通已有输出和 Evidence 写失败纳入自动测试；部署到非本地 FS 前做 replace 语义验证。当前：合并 residual、deferred，不阻塞 Task 7。 |
...
- R-012 合并记录 Task 7 的环境与未自动回归边界；symlink 缺陷本身已关闭，不得把本地 replace 结果外推为强杀或非本地文件系统原子性保证。
