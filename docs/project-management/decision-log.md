...
- Task 3 的 semantic/canonical 顺序和 surrogate 错误边界已验证，R-008 已关闭；drift 和阶段出口仍需任务证据。
- Task 4 的 Compiler 与语义 Source Map 子范围已验证；drift、Runtime 和阶段出口仍需后续任务证据。
- Task 6 的 Evidence/Manifest 子范围和远端同步已验证；Pipeline 已覆盖正常提交、非法定义、Adapter 失败与 symlink 占用，普通已有输出、Evidence 写入中途失败、drift 和阶段出口仍需后续任务证据。
